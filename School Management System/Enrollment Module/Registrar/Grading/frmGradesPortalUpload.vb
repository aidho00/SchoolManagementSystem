...
#End Region
    ...
    Private Sub UpdateDatabaseForRow(row As DataGridViewRow)
        ...
        cn2.Close()
        cn2.Open()
        cm2 = New MySqlCommand("SELECT sg_grade from tbl_students_grades where sg_student_id = '" & row.Cells(4).Value & "' and sg_period_id = '" & row.Cells(2).Value & "' and sg_class_id = '" & row.Cells(1).Value & "' and sg_grade_status IN ('W', 'D')", cn2)
