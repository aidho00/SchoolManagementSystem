...
Public Class frmSupplyPRRecords
    Sub PurchaseRequestList()
        ...
        Dim i As Integer
        Dim sql As String
        sql = "Select prno, prtotal, status, DATE_FORMAT(prdate, '%m/%d/%Y') as prdate, AccountName, prremarks from tbl_supply_purchaserequest pr JOIN useraccounts ua ON pr.pruser_id = ua.useraccountID where prno = '" & frmMain.txtSearch.Text & "' "
        cn.Close()
        cn.Open()
    ...
    Private Sub dgPRList_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgPRList.CellContentClick
        ...
        If colname = "colView" Then
            Try
                ...
                load_datagrid("SELECT t1.`itemid`,t2.description, '','', t1.`itemqty`,t1.itemprice`,t1.`itemtotal` FROM `tbl_supply_purchaserequest_items` t1 JOIN tbl_supply_item t2 ON t1.prno = t2.barcodeid JOIN WHERE `prno` = '" & dgPRList.CurrentRow.Cells(1).Value & "'", dg_report)
                ...
                rptdoc.SetParameterValue("requestdate", Format(Convert.ToDateTime(dgPRList.CurrentRow.Cells(6).Value), "MMMM d, yyyy"))
