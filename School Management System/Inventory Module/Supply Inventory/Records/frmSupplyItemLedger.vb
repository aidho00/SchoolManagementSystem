...
Public Class frmSupplyItemLedger
    ...
    Sub SupplyItemList()
        ...
        Dim i As Integer
        Dim sql As String
            sql = "SELECT (BarcodeID) AS 'Barcode', Description, (CategoryName) AS 'Category', Sizes, (item_price) AS 'Price', cfcissmsdb_supply.tbl_supply_category.catid AS CATID, cfcissmsdb_supply.tbl_supply_sizes.sizeid AS SIZEID, cfcissmsdb_supply.tbl_supply_item.item_open_stock AS OpenStock, cfcissmsdb_supply.tbl_supply_item.item_reorder_point AS ReOrderPoint, cfcissmsdb_supply.tbl_supply_category.categorytype AS SupplyType FROM cfcissmsdb_supply.tbl_supply_item JOIN cfcissmsdb_supply.tbl_supply_category ON cfcissmsdb_supply.tbl_supply_item.CategoryID = cfcissmsdb_supply.tbl_supply_category.catid JOIN cfcissmsdb_supply.tbl_supply_sizes ON cfcissmsdb_supply.tbl_supply_item.sizesid = cfcissmsdb_supply.tbl_supply_sizes.sizeid JOIN cfcissmsdb_supply.tbl_supply_brand ON cfcissmsdb_supply.tbl_supply_item.brandid = cfcissmsdb_supply.tbl_supply_brand.brandid WHERE (BarcodeID LIKE '%" & txtSearch.Text & "%' OR CategoryName LIKE '%" & txtSearch.Text & "%' OR Description LIKE '%" & txtSearch.Text & "%' OR Sizes LIKE '%" & txtSearch.Text & "%' OR brandname LIKE '%" & txtSearch.Text & "%')"
            cn.Close()
        cn.Open()
    ...
    Sub SupplyLedger()
        Try
            ...
            Dim i As Integer
            Dim sql As String
            sql = "Select sl_itembarcode, sl_transaction_type, sl_reference_no, sl_remark, DATE_FORMAT(sl_date, '%m/%d/%Y') as sl_date, sl_stockin_added, sl_stockout_deducted, sl_running_balance from cfcissmsdb_supply.tbl_supply_ledger where sl_itembarcode = '" & lblBarcode.Text & "'"
            cn.Close()
            cn.Open()
            cm = New MySqlCommand(sql, cn)
            dr = cm.ExecuteReader
